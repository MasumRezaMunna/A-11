    ...
    <div className="max-w-2xl mx-auto my-12 p-8 bg-white rounded-3xl shadow-sm border border-slate-100">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input type="text" placeholder="e.g. Need Math Tutor for Class 10" className="w-full p-3 rounded-xl border border-slate-200"
            onChange={(e) => setFormData({...formData, title: e.target.value})} required />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <input type="text" placeholder="Class" className="p-3 rounded-xl border border-slate-200"
            onChange={(e) => setFormData({...formData, class: e.target.value})} required />
          <input type="text" placeholder="Subject" className="p-3 rounded-xl border border-slate-200"
            onChange={(e) => setFormData({...formData, subject: e.target.value})} required />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <input type="text" placeholder="Location" className="p-3 rounded-xl border border-slate-200"
            onChange={(e) => setFormData({...formData, location: e.target.value})} required />
          <input type="number" placeholder="Salary (BDT)" className="p-3 rounded-xl border border-slate-200"
            onChange={(e) => setFormData({...formData, salary: e.target.value})} required />
        </div>
        <textarea placeholder="Tell us more about your requirements..." className="w-full p-3 rounded-xl border border-slate-200 h-32"
