    ...
    >
      ...
      <div className="bg-white rounded-[2.5rem] p-10 shadow-2xl shadow-slate-200/50 border border-slate-50">
        ...
        <div className="flex flex-col md:flex-row items-center gap-10 p-8 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200">
          <div className="relative group">
            ...
            <label
              htmlFor="fileInput"
              className="absolute -bottom-2 -right-2 bg-blue-600 text-white p-3 rounded-2xl cursor-pointer shadow-lg hover:bg-blue-700 transition-all"
            >
              <svg
                className="w-5 h-5"
                fill="none"
          ...
          <div className="flex-1 space-y-4">
            ...
            <p className="text-slate-500 text-sm leading-relaxed">
              Ensure your photo is professional to increase hire rate by up to
              40%.
            </p>
            <input
              type="file"
      ...
      <section>
        <h1 className="text-2xl font-black mb-6 flex items-center gap-3">
          <span className="p-2 bg-amber-100 rounded-lg text-lg">📋</span> Job
          Applications
        </h1>
        {applications.length > 0 ? (
          <div className="grid gap-4">
            {applications.map((app) => (
              <div
                key={app._id}
                className="bg-white p-6 rounded-[1.5rem] border border-slate-100 shadow-sm flex justify-between items-center"
              >
                <div>
                  <h3 className="font-bold text-slate-900">
                    {app.tuition?.title || "Untitled Tuition"}
                  </h3>
                  <p className="text-xs text-slate-500">
                    {app.tuition?.subject} • {app.status}
                  </p>
                </div>
              </div>
            ))}
          ...
          <div className="bg-white p-10 rounded-[2rem] border border-dashed text-center text-slate-400">
            No applications yet.
          </div>
        )}
      </section>
      ...
      <section>
        <h1 className="text-2xl font-black mb-6 flex items-center gap-3">
          <span className="p-2 bg-blue-100 rounded-lg text-lg">📩</span> Direct
          Requests
        </h1>
        {requests.length > 0 ? (
            ...
            {requests.map((req) => (
              ...
              >
                ...
                <button
                  onClick={() =>
                    Swal.fire({
                      title: `Contact ${req.student?.name}`,
                      text: `Email: ${req.student?.email}`,
                      icon: "info",
                    })
                  }
